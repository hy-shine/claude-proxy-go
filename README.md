...
[![Go Version](https://img.shields.io/github/go-mod/go-version/1rgs/claude-code-proxy-go)](https://github.com/1rgs/claude-code-proxy-go)
[![Build Status](https://img.shields.io/github/actions/workflow/status/1rgs/claude-code-proxy-go/ci.yml)](https://github.com/1rgs/claude-code-proxy-go/actions)
[![Docker Image](https://img.shields.io/docker/image-size/1rgs/claude-code-proxy-go/latest)](https://hub.docker.com/r/1rgs/claude-code-proxy-go)
[![License](https://img.shields.io/github/license/1rgs/claude-code-proxy-go)](LICENSE)
...
| `providers.<name>.api_type` | string | Protocol type (currently only `openai`) | `openai` |
| `providers.<name>.api_key` | string | API key for the provider | required |
| `providers.<name>.base_url` | string | Provider API base URL | `https://api.openai.com/v1` |
...
| `models.<model_id>.max_tokens` | int | Max tokens for this model | 16384 |
| `retry.max_retries` | int | Maximum retry attempts | 2 |
| `retry.initial_backoff_ms` | int | Initial backoff in milliseconds | 200 |
| `retry.max_backoff_ms` | int | Maximum backoff in milliseconds | 800 |
| `timeout.request_seconds` | int | Request timeout in seconds | 60 |
| `timeout.stream_seconds` | int | Streaming timeout in seconds | 300 |
...
| `providers.<name>.api_type` | string | 协议类型（当前仅支持 `openai`） | `openai` |
| `providers.<name>.api_key` | string | Provider API 密钥 | 必填 |
| `providers.<name>.base_url` | string | Provider API 基础 URL | `https://api.openai.com/v1` |
| `providers.<name>.models` | object | 模型配置 | 必填 |
| `models.<model_id>.name` | string | 上游模型名称 | 必填 |
| `models.<model_id>.enabled` | bool | 模型是否启用 | `true` |
| `models.<model_id>.max_tokens` | int | 模型最大 token 数 | 16384 |
| `retry.max_retries` | int | 最大重试次数 | 2 |
| `retry.initial_backoff_ms` | int | 初始退避时间（毫秒） | 200 |
| `retry.max_backoff_ms` | int | 最大退避时间（毫秒） | 800 |
| `timeout.request_seconds` | int | 请求超时（秒） | 60 |
| `timeout.stream_seconds` | int | 流式超时（秒） | 300 |
